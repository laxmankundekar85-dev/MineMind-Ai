...
import { Subsidiary, User as AppUser } from '../types';
import { getSupabase, supabase } from '../supabaseClient';
import { GoogleConsoleModal } from './GoogleConsoleModal';
import { 
  getSavedGoogleClientId, 
  promptGoogleLoginDirect, 
  fetchServerGoogleConfig,
  promptNativeGoogleAccount
} from '../googleAuth';
import { 
  Lock, 
  Mail, 
...
export const LoginScreen: React.FC = () => {
  const { loginWithCredentials, submitAccessRequest, requestPasswordReset, login, allUsers } = useApp();

  // Navigation mode within Auth
  const [viewMode, setViewMode] = useState<AuthViewMode>('login');

  // Google Console Modal State
  const [isGoogleConsoleModalOpen, setIsGoogleConsoleModalOpen] = useState<boolean>(false);
  ...
  const handleGoogleSuccessLogin = (profile: { name: string; email: string; picture?: string }) => {
    const userEmail = (profile.email || '').toLowerCase().trim();
    if (!userEmail) {
      setLoginError({ message: 'No Google email address received. Please try again.' });
      return;
    }
    const knownUser = allUsers?.find(u => u.email.toLowerCase() === userEmail);
    const isKnownAdmin = 
      userEmail === 'priyadike23@gmail.com' || 
      userEmail === 'vedantsdike@gmail.com' ||
    ...
    const googleUser: AppUser = {
      id: knownUser?.id || `goog_${Date.now()}`,
      name: profile.name || knownUser?.name || defaultName || 'Google User',
      email: userEmail,
      role: isKnownAdmin ? 'admin' : (knownUser?.role || 'employee'),
      subsidiary: (knownUser?.subsidiary as Subsidiary) || 'CMPDI HQ',
      department: knownUser?.department || (isKnownAdmin ? 'Central Directorate' : 'Geology & Exploration'),
      designation: knownUser?.designation || (isKnownAdmin ? 'Chief Directorate Officer' : 'Senior Technical Officer'),
      employeeId: knownUser?.employeeId || `EMP-G${Date.now().toString().slice(-4)}`,
      status: 'approved',
    };

    try {
      localStorage.setItem('minemind_recent_google_email', userEmail);
    } catch {}

    login(googleUser);
  };

  // Direct trigger function for Google OAuth from modal or main button
  const handleTriggerDirectGoogleLogin = async (clientId: string) => {
    const profile = await promptGoogleLoginDirect(clientId);
    handleGoogleSuccessLogin(profile);
  };

  // Handle Google OAuth Sign-in: Direct sign-in without modal interruption
  const handleGoogleSignIn = async () => {
    ...
    // In Android APK: Use real Android native Google account chooser on the user's phone!
    if (isCapacitorNative) {
      try {
        const result = await promptNativeGoogleAccount();
        ...
        console.debug('[Native Google Auth] User dismissed or error:', nativeErr);
        const msg = String(nativeErr?.message || nativeErr || '').toLowerCase();
        // User cancelled or backed out of native dialog
        if (msg.includes('cancel') || msg.includes('dismiss') || msg.includes('no account')) {
          setIsSubmitting(false);
          return;
        }
        // Fallback to manual entry modal if native dialog could not be invoked
        setIsSubmitting(false);
        setGoogleConsoleError(nativeErr?.message || 'Android account chooser could not be opened.');
    ...
    try {
      // 1. Check for Google Client ID from environment or user-saved settings
      let clientId = getSavedGoogleClientId();
      if (!clientId) {
        clientId = await fetchServerGoogleConfig();
      }

      // 2. If a Google Client ID exists in web browser, directly trigger official Google popup
      if (clientId) {
        try {
          await handleTriggerDirectGoogleLogin(clientId);
          setIsSubmitting(false);
          return;
        } catch (directErr: any) {
          console.debug('[Google Auth] Direct Google GIS flow notice, opening account selector:', directErr?.message || directErr);
          const isCancelled = directErr?.message?.toLowerCase().includes('closed') || directErr?.message?.toLowerCase().includes('cancel');
          if (isCancelled) {
            setIsSubmitting(false);
            return;
          }
          // If popup failed due to origins/environment, open the Google account selector modal
          setIsSubmitting(false);
          setGoogleConsoleError(directErr?.message);
          setIsGoogleConsoleModalOpen(true);
          return;
        }
      }

      // 3. If no Client ID configured, open the account selector modal
      setIsSubmitting(false);
      setIsGoogleConsoleModalOpen(true);
    } catch (err: any) {
      console.debug('[Google Sign-In] Notice:', err?.message || err);
      setIsSubmitting(false);
      setIsGoogleConsoleModalOpen(true);
    } finally {
      setIsSubmitting(false);
  ...
  return (
    ...
    >
          ...
          {viewMode === 'login' && (
            <div className="space-y-5">
              ...
              <button
                ...
                disabled={isSubmitting}
                className="w-full py-2.5 bg-white/95 hover:bg-white text-slate-800 font-medium text-xs sm:text-sm rounded-xl border border-slate-300 shadow-xs flex items-center justify-center gap-2.5 transition-all cursor-pointer hover:shadow-sm active:scale-[0.99] disabled:opacity-60"
              >
      ...
      {/* Google Cloud Console Setup & Direct Access Modal */}
      <GoogleConsoleModal
        isOpen={isGoogleConsoleModalOpen}
        onClose={() => setIsGoogleConsoleModalOpen(false)}
        onSuccessLogin={handleGoogleSuccessLogin}
        onTriggerDirectGoogleLogin={handleTriggerDirectGoogleLogin}
        currentError={googleConsoleError}
      />
    </div>
  );
};
