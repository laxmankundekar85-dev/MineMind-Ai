...
export async function askGroundedKnowledge(
  ...
  // If no chunks match the query within the local document corpus, consult MineMind Domain AI
  if (topMatches.length === 0) {
    console.log(`[xAI Grok RAG] No local chunks found for: "${question}". Synthesizing authoritative mining domain response.`);
    const geminiAI = getGeminiInstance();
    if (geminiAI) {
      const candidateDomainModels = ['gemini-3.6-flash', 'gemini-3.7-flash', 'gemini-3.1-flash-lite'];
      for (const model of candidateDomainModels) {
        try {
          const prompt = `You are MineMind AI, the official technical intelligence platform for CMPDI and Coal India Limited (Ministry of Coal).
The user asked: "${question}".
Provide a comprehensive, authoritative mining engineering answer with DGMS CMR 2017 standards, statutory guidelines, or technical definitions.
Respond strictly in valid JSON format:
{
  "foundInKnowledgeBase": true,
  "answer": "Detailed, professional technical answer...",
  "aiSummary": "One concise executive summary sentence.",
  "confidence": 92,
  "draftOfficialReply": "Formal draft reply suitable for official CMPDI / CIL correspondence."
}`;
          ...
          if (parsed.answer) {
            return {
              ...
              aiSummary: parsed.aiSummary || parsed.answer.slice(0, 150),
              confidence: parsed.confidence || 92,
              draftOfficialReply: parsed.draftOfficialReply || parsed.answer,
              citations: [],
              provider: 'minemind-domain-ai',
            };
          }
        } catch (mErr) {
          console.warn(`[Domain AI] Model ${model} notice:`, mErr);
        }
      }
    }
